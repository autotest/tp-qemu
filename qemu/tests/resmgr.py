...
def run(test, params, env):
    ...
    cp_volume_uuid = resmgr.create_resource_object_from_source(
    ...
    neg(resmgr.bind_resource_object, resource_id=cp_volume_uuid, node_names=["node2"])

    test.log.info("Bind the copied resource to node1")
    neg(resmgr.bind_resource_object, resource_id=cp_volume_uuid, node_names=["node1"])
