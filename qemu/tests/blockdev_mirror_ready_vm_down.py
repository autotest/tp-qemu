...
class BlockdevMirrorReadyVMDownTest(BlockdevMirrorNowaitTest):
        ...
        def _wait_mirror_job_completed(jobid):
            ...
            for i in range(tmo):
                ...
                job_events = [e for e in completed_events if e.get(
                    'data') and jobid in (e['data'].get('id'),
                                          e['data'].get('device'))]
                if job_events:
                    break
                time.sleep(1)
    ...
    def wait_mirror_jobs_cancelled(self):
        ...
        def _wait_mirror_job_cancelled(jobid):
            ...
            for i in range(tmo):
                events = self.main_vm.monitor.get_events()
                cancelled_events = [e for e in events if e.get(
                    'event') == job_utils.BLOCK_JOB_CANCELLED_EVENT]
                job_events = [e for e in cancelled_events if e.get(
                    'data') and jobid in (e['data'].get('id'),
                                          e['data'].get('device'))]
                if job_events:
                    break
