...
def run(test, params, env):
    ...
    def test_migration():
        def check_os_dir_status(virtiofsd_daemon):
            fs = params["filesystems"]
            fs_params = params.object_params(fs)
            fs_dest = fs_params["fs_dest"]
            error_context.context(
            ...
            if virtiofsd_daemon == "source":
                if os.access(fs_dest, os.R_OK):
                    test.fail(
                        "Mount dir %s is accessible after killing source "
                ...
                if not os.access(fs_dest, os.R_OK):
                    test.fail(
        ...
        def kill_source_virtiofsd():
            ...
            if daemon == "source":
                ...
                test.fail(f"Unknown daemon type: {daemon}")
            ...
            check_os_dir_status(virtiofsd_daemon=daemon)
