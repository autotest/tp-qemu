...
def run(test, params, env):
    ...
    clusters_cpus_list = session.cmd_output(
        check_cluster_cpus_list).strip().splitlines()
    if len(clusters_id) != int(vcpu_clusters):
        test.fail("cluster_id is not right: %d != %d"
                  % (len(clusters_id), int(vcpu_clusters)))
    if len(clusters_cpus_list) != int(vcpu_sockets)*int(vcpu_clusters):
        test.fail("cluster_cpus_list is not right: %d != %d"
