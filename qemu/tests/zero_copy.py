...
def run(test, params, env):
    ...
    def enable_zerocopytx_in_host(enable=True):
        ...
        if process.system(cmd, shell=True) or enable != zerocp_enable_status():
            raise exceptions.TestSkipError("Set vhost_net zcopytx failed")

    error_context.context("Set host vhost_net experimental_zcopytx", logging.info)
    if params.get("enable_zerocp", 'yes') == 'yes':
        enable_zerocopytx_in_host()
    else:
        enable_zerocopytx_in_host(False)

    error_context.context("Boot vm with 'vhostforce=on'", logging.info)
    params["vhost"] = "vhostforce=on"
    params["start_vm"] = 'yes'
    ...
    error_context.context("Check guest is alive and have no crash", logging.info)
    vm.verify_alive()
