...
def run(test, params, env):
    ...
    Steps:
        1. Setup a local nfs server and mount it to dir fs and targetfs
        2. Run the virtiofsd daemon to share fs with different cache modes
        3. Boot the source guest with the virtiofs device in step2
        4. Mount the virtiofs targets inside the guest
        5. Create a file and get its md5, then run fio
        6. Run the virtiofsd daemon to share targetfs
        7. Boot the target guest with the virtiofs device in step6
        8. Do migration from the source guest to the target guest
        9. No error occurs, the virtiofs is mounted automatically and
           the file md5 should keep the same, fio is still running
    ...
    def start_io(session):
        def do_fio():
            fs_dest = guest_mnts[fs_target]
            error_context.context("Start fio on %s." % fs_dest, test.log.info)

            fio = generate_instance(params, vm, "fio")
            guest_fio_objects[fs_target] = fio
            tmo = params.get_numeric("fio_runtime", 1800)
            bg_test = utils_test.BackgroundTest(
                fio.run, (params["fio_options"] % fs_dest, tmo)
            )
            bg_test.start()

        def dd_file():
            error_context.context("Create a file and get its md5", test.log.info)
            fs_dest = guest_mnts[fs_target]
            guest_file = fs_params["guest_file"] % fs_dest
            io_timeout = params.get_numeric("io_timeout", 300)
            session.cmd(params["cmd_dd"] % guest_file, io_timeout)

            cmd_md5 = params["cmd_md5"] % fs_dest
            md5 = session.cmd_output(cmd_md5, io_timeout).strip().split()[0]
    ...
    def stop_io():
        error_context.context("Stop fio", test.log.info)
        for fio in guest_fio_objects.values():
            fio.clean(force=True)
    ...
    def test_migration():
        def check_fio_running():
            ...
            for fs in params.objects("filesystems"):
                fs_params = params.object_params(fs)
                fio_name = fs_params["fio_name"]
                out = session.cmd_output(params["cmd_chk_fio"])
                test.log.debug("Status of fio process: %s", out)

                if fio_name not in out:
                    test.fail(f"Failed to get a running fio process for {fs}")
