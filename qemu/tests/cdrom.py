...
def run(test, params, env):
    ...
    def get_cdrom_file(vm, qemu_cdrom_device):
        ...
        enable_blockdev = Flags.BLOCKDEV in vm.capabilities
        cdfile = None
        if isinstance(blocks, six.string_types):
            ...
            for block in blocks:
                if (enable_blockdev and block['qdev'] == vm.devices.get_qdev_id(
                        qemu_cdrom_device) or (not enable_blockdev and
                                               block['device'] == qemu_cdrom_device)):
                    try:
    ...
    def get_empty_cdrom_device(vm):
        ...
        if isinstance(blocks, six.string_types):
            ...
            for block in blocks:
                if Flags.BLOCKDEV in vm.capabilities:
