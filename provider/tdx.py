...
class TDXDcap(object):
        ...
        try:
            ...
            try:
                ...
                if sock.connect_ex(("localhost", int(pccs_port))) == 0:
                    raise exceptions.TestFail("Port %s is already in use" % pccs_port)
            finally:
                sock.close()
    ...
    def verify_dcap_services(self, services, fail_on_inactive=True):
        ...
        :return: When fail_on_inactive is False, returns True if all services are
                 active, False otherwise. When fail_on_inactive is True, returns None.
        """
        if fail_on_inactive:
        ...
        return True if not fail_on_inactive else None
