...
def run(test, params, env):
    ...
    def test_readonly():
        def check_file_content():
            ...
            out = session.cmd_output(params["read_file_cmd"]).strip()
        ...
        def write_file():
            ...
            output = session.cmd_output(params.get("write_file_cmd"))
            test.log.info(output)
            if os_type == "linux":
                if params.get("check_str_linux") not in output:
                    test.fail("The mounted dir is not read-only.")
            else:
                if params.get("check_str_win") not in output:
                    test.fail("The mounted dir is not read-only.")
