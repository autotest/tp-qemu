...
def run(test, params, env):
    ...
    def verify_elapsed_time():
        ...
        if times_list:
            test.fail("Unexpected time drift found:"
                      " Detail: '%s' \n timelist: %s" % (output, times_list))

    error_context.context("Sync the host system time with ntp server", logging.info)
    ntp_sync_cmd = "yum install -y ntpdate; ntpdate clock.redhat.com"
    ...
    if len(cpu_pin_list) < len(vm.vcpu_threads):
        test.error("There isn't enough physical cpu to"
                   " pin all the vcpus")
    check_one_cpu_pinned = False
    for vcpu, pcpu in cpu_pin_list:
        pin_vcpu_cmd = "taskset -p -c %s %s" % (pcpu, vcpu)
        process.system(pin_vcpu_cmd, shell=True)
