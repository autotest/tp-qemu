...
def _get_epc_size(output):
    ...
    for line in output.splitlines():
        tmp_epc = re.search(r'\b0x[0-9a-fA-F]*-0x[0-9a-fA-F]*\b',
                            str(line))
        if tmp_epc:
            epc_size += int(tmp_epc[0].split('-')[1], 16) - int(
                tmp_epc[0].split("-")[0], 16) + 1
    return epc_size
...
class SGXChecker(object):
    ...
    def get_config_total_epc_size(self):
        ...
        for ele in epc_list:
            ...
            ele_memdev = self._params.object_params(
                epc_params["vm_sgx_epc_memdev"])
            tmp_epc_size = int(float(normalize_data_size(
                ele_memdev["size_mem"], "B", 1024)))
    ...
    def verify_guest_epc_size(self, cmd_output):
        ...
        if guest_msg_epc_size != int(guest_total_epc_size):
            self._test.fail("Guest epc size %s is not equal to qemu set "
                            "section size %s" % (guest_msg_epc_size,
                                                 guest_total_epc_size))
