...
def run(test, params, env):
    ...
    def uninstall_by_installer(session):
        ...
        time.sleep(30)
        s_check, o_check = session.cmd_status_output(
            installer_pkg_check_cmd)
        if s_check == 0:
    ...
    def repair_by_installer(session):
        ...
        msi_file = params["msi_file"]
        msi_path = drive_letter + "\\" + msi_file
        ...
        for driver_name, device_name, device_hwid in zip(
            ...
            if driver_name == "viofs":
                error_context.context("Check %s service." % driver_name,
                                      test.log.info)
                output = virtio_fs_utils.query_viofs_service(test,
                                                             params,
                                                             session)
