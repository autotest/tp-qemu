...
def run(test, params, env):
    ...
    def transfer_data(src, dst):
        error_context.context("Transferring data from %s to %s" %
                              (src.name, dst.name), test.log.info)
        sessions[dst].sendline(receive_cmd % (guest2guest_port, dest_path))
        time.sleep(5)
        sessions[src].sendline(sent_cmd % (guest_path, gateway, guest2guest_port))
        dst_md5 = get_file_md5sum(dest_path, sessions[dst],
                                  timeout=file_md5_check_timeout)
        error_context.context("md5 value of data in %s: %s" % (dst.name, dst_md5),
                              test.log.info)
        if dst_md5 != src_md5:
    ...
    try:
        ...
        host2guest_port = int(re.search(r'\d+', port_list[0]).group())
        for vm in vms:
            ...
            host2guest_port = int(re.search(r'\d+', port_list[1]).group())

        guest2guest_port = int(re.search(r'\d+', port_list[1]).group())
        transfer_data(*vms)
        guest2guest_port = int(re.search(r'\d+', port_list[0]).group())
