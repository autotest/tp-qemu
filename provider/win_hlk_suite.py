...
class HLKServer(object):
    ...
    def get_machine_target_key(self, target_name, machine_name, pool_name, timeout=60):
        ...
        :type: str
        """
        return self.get_machine_target(
                target_name, machine_name, pool_name, timeout)[1].replace('&', '"&"')
    ...
    def create_project_target(self, target_key, project_name,
                              machine_name, pool_name, timeout=60):
        """
    ...
    def list_tests(self, target_key, project_name, machine_name, pool_name, timeout=60):
        ...
        :rtype: list
        """
        cmd = 'listtests %s %s %s %s' % (target_key, project_name,
                                         machine_name, pool_name)
        tests = self._session.cmd_output(cmd, timeout)
        return [json.loads(test) for test in tests.splitlines()]

    def get_target_test(self, test_name, target_key, project_name,
                        machine_name, pool_name, timeout=60):
        """
        Get target test.
        ...
        :rtype: dict
        """
        tests = self.list_tests(target_key, project_name,
                                machine_name, pool_name, timeout)
        for test in tests:
            if test['test_name'] == test_name:
                return test

    def get_target_test_id(self, test_name, target_key, project_name,
                           machine_name, pool_name, timeout=60):
        """
        Get target test ID.
    ...
    def zip_test_result_logs(self, result_index, test_id, target_key,
    ...
    def list_test_results(self, test_id, target_key, project_name,
        ...
        :rtype: str
        """
        cmd = 'listtestresults %s %s %s %s %s' % (test_id, target_key,
                                                  project_name, machine_name,
                                                  pool_name)
        return self._session.cmd_output(cmd)

    def list_tests_results(self, tests_id, target_key, project_name,
                           machine_name, pool_name):
        """
        List tests results.
        ...
        for test_id in tests_id:
            output = self.list_test_results(test_id, target_key, project_name,
                                            machine_name, pool_name)
    ...
    def run_tests(self, tests_id, target_key, project_name,
                  machine_name, pool_name, timeout=600, step=3):
        """
        Run tests.
        ...
        """
        for test_id in tests_id:
            self.queue_test(test_id, target_key, project_name,
                            machine_name, pool_name)
        ...
        resutls = self.list_tests_results(tests_id, target_key, project_name,
                                          machine_name, pool_name)
        err_msg = []
    ...
    def simple_run_test(self, pool_name, project_name, target_name,
                        tests_name, timeout=14400, step=600):
        """
        Simple run test.
        ...
        for test_name in tests_name:
            test_id = self.get_target_test_id(test_name, target_key,
                                              project_name, machine_name,
                                              pool_name)
            tests_id.append(test_id)

        self.run_tests(tests_id, target_key, project_name,
                       machine_name, pool_name, timeout, step)
...
def download_hlk_server_image(params, src_img_uri, timeout=1800):
    ...
    if re.search(r'(^http:)|(^https:)|(^ftp")|(^ftps:")', src_img_uri):
        ...
        dst_img = {'image_name': info_dict['filename'].split('.')[0],
