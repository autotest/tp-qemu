...
def block_resize_enlarge(vm, session, params, device, block_size):
    ...
    if params.get("os_type") == "windows":
        utils_misc.extend_shrink_windows_disk(session, params["disk_index"],
                                              params["disk_letter"],
                                              operation="extend")
...
def block_resize_shrink(vm, session, params, device, old_disk_size, shrunk_size):
    ...
    """
    if params.get("os_type") == "windows":
        shrunk_size = utils_misc.extend_shrink_windows_disk(session,
                                                            params["disk_index"],
                                                            params["disk_letter"],
                                                            operation="shrink")
...
def refresh_disk_after_resize(vm, session, params):
    ...
    if params.get("os_type") == "windows":
        utils_misc.rescan_windows_disk(session)
...
def run(test, params, env):
    ...
    block_virtual_size = vm.get_block_virtual_size({'device': data_image_dev})
