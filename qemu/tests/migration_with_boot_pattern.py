...
def run(test, params, env):
    ...
    def wait_for_boot_pattern(boot_pattern, vm):
        ...
        while True:
            try:
                if vm.serial_console.read_until_last_line_matches(patterns, timeout=1):
                    return True
    ...
    post_boot_pattern_cmd = (
        params.get("post_boot_pattern_cmd", b"")
        .encode(astring.ENCODING)
        .decode("unicode_escape")
    )
    login_timeout = int(params.get("login_timeout", 360))

    error_context.context("Start VM and wait for pattern while migrating", logging.info)
    end = time.time() + boot_pattern_timeout
    vm.create()
    thread = InterruptedThread(wait_for_boot_pattern, args=(boot_pattern, vm))
    thread.start()

    try:
        while thread.is_alive():
            assert time.time() < end, (
                "Boot pattern not found in %ss" % boot_pattern_timeout
            )
    ...
    assert bg_result is True, (
        "Background thread failed, check the log for details (%s)" % bg_result
    )

    error_context.context(
        "Additional migration after the pattern is found", logging.info
    )
    vm.migrate(mig_timeout, mig_protocol, mig_cancel_delay, env=env)
    if post_boot_pattern_cmd:
        vm.serial_console.sendline(post_boot_pattern_cmd)
        error_context.context(
            "Second migration after the post-boot-pattern-cmd", logging.info
        )
        vm.migrate(mig_timeout, mig_protocol, mig_cancel_delay, env=env)
