...
class QemuGuestAgentBasicCheck(QemuGuestAgentTest):
    ...
    def gagent_check_install_no_device(self, test, params, env):
        ...
        qemu_ga_pkg = params.get("qemu_ga_pkg")
        install_cmd_fmt = params.get("gagent_install_cmd")

        if not qemu_ga_pkg or not install_cmd_fmt:
            test.fail(
                "Missing required parameters for QGA installation "
                "(qemu_ga_pkg or gagent_install_cmd)"
            )
