...
def run(test, params, env):  # pylint: disable=R0915
    ...
    try:
        ...
        def prepare_pci_passthrough():
            ...
            if not linux_modules.configure_module("vfio-pci", "CONFIG_VFIO_PCI"):
                test.cancel("vfio-pci module can not be configured.")
        ...
        if kvm_probe_module_parameters == "avic=1":
            if not linux_modules.configure_module("msr", "CONFIG_X86_MSR"):
                test.cancel("msr module can not be configured.")
            if mode == "apic":
                check_avic_support()
            if mode == "x2apic":
