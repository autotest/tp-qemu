...
def run(test, params, env):
    ...
    def _record_poolmon():
        ...
        if status:
            test.fail("Fail to get Mmdi pool tag memory "
                      "info in pool monitor.")
    ...
    poolmon_mmdi_cmd = utils_misc.set_winutils_letter(session,
                                                      poolmon_mmdi_cmd)
    ...
    error_context.context("Check the diff of allocation memory and"
                          " free memory for Mmdi pool tag"
                          " in memory pool monitor before"
                          " start io test.", test.log.info)
    ...
    if int(diff_aft) - int(diff_befor) > 100:
        test.fail("There are memory leak on virtiofs,"
