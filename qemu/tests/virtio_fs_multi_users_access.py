...
def run(test, params, env):
    ...
    def get_eth_name(session):
        ...
        eth_name = ipconfig_output.split(match)[0] \
            .split("Ethernet adapter ")[-1] \
            .split(":")[0]
        return eth_name
    ...
    def switch_dynamic_ip_to_static_ip(session, eth_name):
        ...
        :param session: the session of guest
        """
        error_context.context("Get the config of %s." % eth_name,
                              test.log.info)
        net = session.cmd_output("netsh interface ip show config name=\"%s\"" %
                                 eth_name)

        ipaddr, gateway, dns_server, subnet_mask = "", "", "", ""
        ...
        error_context.context("The config will be set to ipaddress:%s, "
                              "gateway:%s, subnet mask:%s." % (
                                  ipaddr, gateway, subnet_mask),
                              test.log.info)

        ip_cmd = "netsh interface ip set address name=\"%s\" source=static " \
                 "addr=%s mask=%s gateway=%s" % (eth_name, ipaddr,
                                                 subnet_mask, gateway)
        session.cmd(ip_cmd)

        dns_cmd = "netsh interface ip set dnsservers \"%s\" static " \
                  "192.168.0.1 primary" % eth_name
        session.cmd(dns_cmd)

    def switch_ip_to_dynamic(session, eth_name):
        if eth_name:
            restore_ip_cmd = "netsh interface ip set address name=\"%s\" " \
                             "source=dhcp" % eth_name
            session.cmd(restore_ip_cmd)
            restore_dns_cmd = "netsh interface ip set dnsservers name=\"%s\" " \
                              "source=dhcp" % eth_name
    ...
    try:
        if not domain_dns:
            error_context.context("Create the user(s) on guest...",
                                  test.log.info)
            for _username, _pwd in zip(username, pwd):
                if os_type == "windows":
                    status = session.cmd_status(add_user_cmd % (_username,
                                                                _pwd))
                else:
                    status = session.cmd_status(add_user_cmd % (_pwd,
                                                                _username))
                if status != 0:
        ...
        if os_type == "windows":
            ...
            error_context.context("Set 777 permission for all users.",
                                  test.log.info)
        ...
        for _username, _pwd in zip(username, pwd):
            try:
                if not domain_dns:
                    error_context.context("Login the user: %s" % _username,
                                          test.log.info)
                    session = vm.wait_for_login(username=_username,
                                                password=_pwd)
                else:
                    session = vm.wait_for_login()
                    ...
                    join_domain_cmd = join_domain_cmd.replace("%s",
                                                              _username,
                                                              1)
                    join_domain_cmd = join_domain_cmd.replace("%s",
                                                              _pwd,
                    ...
                    if "does not exist" in output:
                        ...
                        error_context.context("The netdom is NOT supported, "
                ...
                if os_type == "windows":
                    virtio_fs_utils.basic_io_test_via_psexec(test, params, vm,
                                                             _username, _pwd)
                else:
                ...
                if domain_dns:
                    ...
                    remove_domain_cmd = remove_domain_cmd.replace("%s",
                                                                  _username,
                                                                  1)
                    remove_domain_cmd = remove_domain_cmd.replace("%s",
                                                                  _pwd,
        ...
        if not domain_dns:
            error_context.context("Delete the user(s) on guest...",
                                  test.log.info)
