...
def run(test, params, env):
    ...
    def querey_block_exports(image_params):
        ...
        image_size = utils_numeric.normalize_data_size(
            str(image_info.get("virtual-size")), "G").split('.')[0] + 'G'
        image_name = image_info.get("filename")
        expected_image_name = storage.get_image_filename(image_params,
    ...
    def hotplug_secret_objects():
        ...
        args = {"node-name": "fmt_stg1", "job-id": "job_add_key1",
                "options": {"driver": "luks", "state": "active",
                            "new-secret": "sec1", "keyslot": 1,
                            "iter-time": 10}}
        try:
