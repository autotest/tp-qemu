...
def run(test, params, env):
    ...
    def config_win_before_test(session):
        ...
        """
        error_context.context("Do driver verify and winfsp installation"
                              " in windows guest.", test.log.info)
    ...
    def create_fs_devices(fs_name, fs_params):
        ...
        Create fs devices.
        """
        return vm.devices.fs_define_by_params(
            fs_name, fs_params)
    ...
    def plug_fs_devices(action, plug_devices):
        ...
        for dev in plug_devices:
            error_context.context("%s %s device (iteration %d)" %
                                  (action.capitalize(), dev.get_qid(),
