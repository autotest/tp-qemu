    ...
    class TestMultihostMigration(mig_type, MigrationBase):
        ...
        def __init__(self, test, params, env):
            ...
            if self.sub_type == "before_migrate_cache_size":
                ...
                self.post_migration = \
                    self.post_migration_capability_with_xbzrle_off_on
            if self.sub_type == "after_migrate_cache_size":
                self.before_migration = \
                    self.before_migration_capability_with_xbzrle_on
                self.post_migration = \
        ...
        def get_mig_totaltime_downtime_transferred_ram(self, vm):
            ...
            error.context("Get total time, downtime and transferred ram "
            ...
            transferred_ram = \
                int(vm.monitor.info("migrate").get("ram").get("transferred"))
            ...
            if self.is_src:
                ...
                for i in range(len(mig_total_time_list)):
                    if min(mig_total_time_list) != mig_total_time_list[-1]:
                        raise error.TestFail("The total time of migration is "
                                             "error, %s should be minimum, "
                                             "but actual is %s" %
                                             (mig_total_time_list[-1],
                                              min(mig_total_time_list)))
                    else:
                        mig_total_time_list.pop()
                    if min(mig_downtime_list) != mig_downtime_list[-1]:
                        raise error.TestFail("The downtime of migration is "
                                             "error, %s should be minimum, "
                                             "but actual is %s" %
                                             (mig_downtime_list[-1],
                                              min(mig_downtime_list)))
                    else:
                    ...
                    if min(transferred_ram_list) != transferred_ram_list[-1]:
                        raise error.TestFail(
                            ...
                            (transferred_ram_list[-1],
                             min(transferred_ram_list)))
                    else:
        ...
        @error.context_aware
        def post_migration_capability(self, vm, cancel_delay, mig_offline,
                                      dsthost, vm_ports,
