...
def run(test, params, env):
    ...
    session.cmd("xcopy %s C:\\" % test_tool_src_path)
    session.cmd("C:\\%s -i" % test_tool)
    session = vm.reboot(session)

    LOG_JOB.info("Start OpenSSH service after reboot VM")
    s, o = session.cmd_status_output(params["start_openssh_service"])
    if s != 0:
        test.error("Failed to start openssh service: %s" % o)
