...
def run(test, params, env):
    ...
    def test_migration(session):
        def check_os_status(virtiofsd_daemon, session):
            ...
            # Check the source VM status, so change the VM back
            vm.params["filesystems"] = vm.params["filesystems"]
            ...
            if virtiofsd_daemon == "source":
                ...
                # Check that the mount directory is no longer accessible after 
                # killing virtiofsd
                ...
                if utils_misc.wait_for(
                    ...
                    if not vm.monitor.verify_status("postmigrate"):
                        ...
                        error_context.context("Continue the source VM")
                    else:
                    ...
                    if os_type == "windows":
                        if session.cmd_status(f"dir {fs_dest}\\", timeout=10):
                            test.fail(
                                "Mount dir %s is not accessible after killing target "
                                "virtiofsd process" % fs_dest
                            )
                    else:
                        if session.cmd_status(f"ls {fs_dest}", timeout=10):
                            test.fail(
                                "Mount dir %s is not accessible after killing target "
                                "virtiofsd process" % fs_dest
                            )
        ...
        def kill_virtiofsd_check(session):
            ...
            # Validate the daemon parameter
            daemon = params["daemon"]
            if daemon not in ["source", "target"]:
                test.fail(
                    "Invalid daemon type: %s. Expected 'source' or 'target'." % daemon
                )
        ...
        # Wait until the VM status is 'inmigrate', then proceed
        while True:
