...
class GuestAgentPkg(object):
    ...
    def __init__(self, build_tag, build_name, url, authtype='', server='',
                 topdir='', weburl='', topurl=''):
...
def parse_params(program):
    ...
    option_list = [('build_tag', "the tag of the build"),
                   ('build_name', "the name of the build")]
    brew_conf_list = [('-s', '--server', "url of XMLRPC server"),
    ...
    parser.add_argument('-u', '--url', required=True,  dest='download_url',
...
if __name__ == '__main__':
    ...
    guestagent = GuestAgentPkg(args.build_tag, args.build_name,
                               args.download_url, args.authtype, args.server,
                               args.topdir, args.weburl, args.topurl)
