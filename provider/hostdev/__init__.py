...
PCI_DEV_PATH = os.path.join(PCI_PATH, 'devices')
PCI_DRV_PATH = os.path.join(PCI_PATH, 'drivers')
...
class HostDeviceBindError(HostDeviceError):
    ...
    def __str__(self):
        return (f'Cannot bind "{self.slot_id}" to driver "{self.driver}": '
                f'{self.error}')
...
class HostDeviceUnbindError(HostDeviceBindError):
    ...
    def __str__(self):
        return (f'Cannot unbind "{self.slot_id}" from driver "{self.driver}": '
                f'{self.error}')
...
class PFDevice:
    def __init__(self, slot_id, driver):
        ...
        self.driver = driver
        self.slot_id = slot_id
        self.slot_path = os.path.realpath(
            os.path.join(PCI_DEV_PATH, self.slot_id))
        self.origin_driver = self._get_current_driver(slot_id)
        self.pci_class = open(
            os.path.join(self.slot_path, 'class')).read().strip()[2:4]
        self.device_id = open(
            os.path.join(self.slot_path, 'device')).read().strip()[2:]
        self.vendor_id = open(
            os.path.join(self.slot_path, 'vendor')).read().strip()[2:]
        if self.pci_class == DEV_CLASSES['network'] and self.origin_driver != 'vfio-pci':
            nic_name = os.listdir(os.path.join(PCI_DEV_PATH, slot_id, 'net'))[0]
            with open(os.path.join('/sys/class/net', nic_name, 'address'),
                      'r') as addr_f:
                self.mac_addresses = [addr_f.read().strip()]
    ...
    def _get_current_driver(slot_id):
        ...
        """
        current_driver = os.path.join(os.path.join(PCI_DEV_PATH, slot_id),
                                      'driver')
        if os.path.exists(current_driver):
            return os.path.basename(os.path.realpath(current_driver))
    ...
    def same_iommu_group_devices(self):
        ...
        """
        return os.listdir(
            os.path.join(self.slot_path, 'iommu_group', 'devices'))
    ...
    def bind_one(self, slot_id, driver):
        ...
        if current_driver:
            if current_driver == driver:
                LOG_JOB.info(f'Notice: {slot_id} already bound to driver '
                             f'{driver}, skipping')
                return
        ...
        # match of the device.
        override_file = os.path.join(PCI_DEV_PATH, slot_id, 'driver_override')
        if os.path.exists(override_file):
            try:
                with open(override_file, 'w') as override_f:
                    override_f.write(driver)
            except OSError as e:
                raise HostDeviceError(f'Failed to set the driver '
                                      f'override for {slot_id}:  {str(e)}')
        # For kernels < 3.15 use new_id to add PCI id's to the driver
        else:
            id_file = os.path.join(PCI_DRV_PATH, driver, 'new_id')
            try:
                with open(id_file, 'w') as id_f:
                    id_f.write(f'{self.vendor_id} {self.device_id}')
            except OSError as e:
        ...
        # Bind to the driver
        try:
            with open(os.path.join(PCI_DRV_PATH, driver, 'bind'), 'a') as bind_f:
    ...
    def unbind_one(self, slot_id):
        ...
        if current_driver:
            ...
            driver_path = os.path.join(PCI_DRV_PATH, current_driver)
            try:
                with open(os.path.join(driver_path, 'unbind'), 'a') as unbind_f:
...
class VFDevice(PFDevice):
    def __init__(self, slot_id, driver):
        ...
        self.num_vfs = 0
        self.vfs = []
        self.sriov_numvfs_path = os.path.join(self.slot_path, 'sriov_numvfs')
    ...
    def _config_net_vfs(self, params):
        ...
        """
        self.mac_addresses = []
        dev_name = os.listdir(os.path.join(self.slot_path, 'net'))[0]
        for idx, vf in enumerate(self.vfs):
            mac = params.get(f'hostdev_vf{idx}_mac',
    ...
    def config(self, params):
        ...
        super().config(params)
        with open(os.path.join(self.slot_path, 'class'), 'r') as class_f:
            if class_f.read()[2:4] == '02':
                LOG_JOB.info(f'Device "{self.slot_id}" is a network device, '
                             f'configure MAC address for VFs')
    ...
    def create_vfs(self, counts):
        ...
        if self._get_current_driver(self.slot_id) == 'vfio-pci':
            raise VFCreateError(
                self.slot_id, f'Slot "{self.slot_id}" is bound to '
                              f'"vfio-pci", please fall back to kernel driver '
                              f'to create VFs')
        try:
            with open(os.path.join(self.slot_path, 'sriov_totalvfs'), 'r') as f:
                if counts > int(f.read()):
                    raise VFCreateError(
                        self.slot_id, 'Count of VF to be created is '
                                      'larger than "sriov_totalvfs"')
            with open(self.sriov_numvfs_path, 'w+') as numvfs_f:
                self.num_vfs = int(numvfs_f.read())
                if self.num_vfs != 0:
                    numvfs_f.write('0')
                    numvfs_f.flush()
        ...
        wait.wait_for(
            lambda: len(glob(os.path.join(self.slot_path, 'virtfn*'))) == counts,
            timeout=(counts * 10))

        vfs = sorted(glob(os.path.join(self.slot_path, 'virtfn*')))
        self.vfs = [os.path.basename(os.path.realpath(vf)) for vf in vfs]
