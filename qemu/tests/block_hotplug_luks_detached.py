...
def run(test, params, env):
    """
    Hotplug a LUKS device with a detached header to a running VM (Linux or Windows), do IO, then unplug and verify.
    Steps:
    ...
    disk_op_cmd = params.get("disk_op_cmd", "dd if=%s of=/dev/null bs=1k count=1000 iflag=direct && dd if=/dev/zero of=%s bs=1k count=1000 oflag=direct")
    ...
    try:
        ...
        vm.monitor.cmd("blockdev-add", {"node-name": "libvirt-1-storage", "driver": "file", "filename": luks_payload})
        # blockdev-add for header
        vm.monitor.cmd("blockdev-add", {"node-name": "libvirt-2-storage", "driver": "file", "filename": luks_header})
        # object-add secret
        vm.monitor.cmd("object-add", {"qom-type": "secret", "id": "libvirt-2-storage-secret0", "data": luks_secret})
        # blockdev-add for raw
        vm.monitor.cmd("blockdev-add", {"node-name": "libvirt-1-format", "driver": "raw", "file": "libvirt-1-storage"})
        # blockdev-add for luks
        vm.monitor.cmd("blockdev-add", {"node-name": "libvirt-2-format", "driver": "luks", "file": "libvirt-1-format", "header": "libvirt-2-storage", "key-secret": "libvirt-2-storage-secret0"})
        # device_add
        vm.monitor.cmd("device_add", {"num-queues": "1", "driver": "virtio-blk-pci", "drive": "libvirt-2-format", "id": "virtio-disk2"})
    except QMPCmdError as e:
        test.fail(f"QMP hotplug failed: {e}")
    ...
    if windows:
        ...
        test.log.info(f"New disk detected (Windows index): {new_disk}")
        ...
        if disk_index and disk_letter:
            ...
            drive_letter = utils_disk.configure_empty_windows_disk(session, new_disk, params.get("luks_payload_size", "5G"))[0]
        test_cmd = params.get("disk_op_cmd", "") % (drive_letter, drive_letter)
        ...
        if not new_disks:
            test.fail("No new disk detected after hotplug!")
        new_disk = new_disks[0]
        test.log.info(f"New disk detected: {new_disk}")
    ...
    try:
        ...
        def disk_gone():
            if windows:
                return new_disk not in session.cmd("wmic diskdrive get index").split()[1:]
            else:
