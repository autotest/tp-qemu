...
def run(test, params, env):
    ...
    try:
        ...
        for x in range(int(nums_cpu)):
            ...
            params['chardev_path_vs{}'.format(x)] = "{}trace-path-cpu{}".format(v_path, x)
        ...
        status, output = session.cmd_status_output("echo 1 > /sys/kernel/debug/tracing/events/sched/enable")
        if status != 0:
        ...
        for index, proc in enumerate(procs):
            if not proc.get_output():
                test.fail("cpu %s do not have output while it is enabled in host" % index)
            proc.close()
