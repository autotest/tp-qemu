...
def run(test, params, env):
    ...
    error_context.context("Check virtio-win-installer version.",
                          LOG_JOB.info)
    pkg_status = process.getstatusoutput(params["rpm_install_chk_cmd"],
                                         shell=True)[0]
    if pkg_status:
        test.cancel("Pls check the test env: whether virtio-win pkg is "
                    "installed on host.")

    pkg_ver = process.system_output(params["rpm_ver_chk_cmd"],
                                    shell=True).strip().decode()

    iso_name = process.system_output(params["iso_name_chk_cmd"],
                                     shell=True).strip().decode()
    ...
    installer_ver = session.cmd_output(params["installer_chk_cmd"]
                                       % vol_virtio).strip()
    if not pkg_ver == iso_ver == iso_label_ver == installer_ver:
        test.fail("Installer version isn't the same with others,"
