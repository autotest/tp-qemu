...
def run(test, params, env):
    ...
    if not host_phys_bits.isdigit():
        test.error("Failed to get host phys-bits, the actual output is '%s'"
                   % host_phys_bits)
    ...
    try:
        error_context.context("Start the vm with host-phys-bits-limit=%s."
                              % host_phys_bits_limit, test.log.info)
        ...
        if err_msg:
            ...
            if not re.search(ignored_err_msg, str(e), re.S | re.I):
                test.fail("Boot a vm with phys-bits '%s', the ignored "
        ...
        if err_msg:
            test.fail("Start the vm unexpectedly with "
