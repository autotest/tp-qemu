...
def execute_io_test(
    ...
    try:
        ...
        if clean:
            ...
            if not is_windows:
                utils_disk.is_mount(did, mount_point, fstype=fstype, session=session)
                ...
                session.cmd(f'"RD /S /Q "{mount_point}"')
                utils_disk.clean_partition(session, mount_point, os_type)
