...
def run(test, params, env):
    ...
    def iface_check(vm, session):
        ...
        iface_info = [iface for iface in iface_info if
                      iface['link_type'] == 'ether']
        phy_nics = vm.params['vm_hostdev_slots']
        if len(iface_info) != len(phy_nics):
            test.fail(f'Unexpected number of interfaces: Assigned '
    ...
    def lifecycle(cycle_type, method, vm, session):
        ...
        if cycle_type == 'shutdown':
            ...
            status, _ = utils_net.ping(utils_net.get_host_ip_address(params),
                                       10,
                                       timeout=30,
                                       session=session)
            if status:
                test.fail('Failed to ping from VM to host after reboot with '
    ...
    with hostdev_setup(params) as params:
        ...
        for vm in env.get_all_vms():
            ...
            pci_slots = []
            error_context.base_context(f'Setting hostdevs for {vm.name}',
                                       test.log.info)
