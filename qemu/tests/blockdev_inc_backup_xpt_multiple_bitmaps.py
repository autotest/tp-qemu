...
class BlockdevIncbkXptMutBitmaps(BlockdevLiveBackupBaseTest):
    ...
    def check_nbd_export_info(self):
        if self._block_export_uid is not None:
            ...
            if (not info or info['shutting-down'] or
                    info['id'] != self._block_export_uid or
                    info['type'] != 'nbd' or
                    info['node-name'] != self._full_bk_nodes[0]):
                self.test.fail(
    ...
    def add_persistent_bitmaps(self):
        ...
        bitmaps = [{'node': self._full_bk_nodes[0],
                    'name': b, 'persistent': True, 'disabled': s}
