...
def run(test, params, env):
    ...
    def get_user_uid_gid(user_name):
        ...
        """
        output = process.system_output("id %s" % user_name,
    ...
    def enable_uid_gid(uid_gid_value):
        ...
        if s:
            test.fail("Fail command: %s. Output: %s." %
                      (enable_cmd, o))
        error_context.context("Restart virtiofs service after modify"
                              " the registry.", test.log.info)
    ...
    def check_file_uid_gid(volume_letter, shared_dir, expect_id):
        ...
        error_context.context("Check the file's UID and GID on host.",
                              test.log.info)
        host_file = os.path.join(shared_dir, file_name)
        output = process.system_output("ls -l %s" % host_file,
                                       shell=True).decode().strip()
        ...
        if process.system("id %s -u" % owner, shell=True, ignore_status=True):
            ...
            uid = process.system_output("id %s -u" % owner,
                                        shell=True).decode().strip()
        if process.system("id %s -g" % group, shell=True, ignore_status=True):
            gid = group
        else:
            gid = process.system_output("id %s -g" % group,
                                        shell=True).decode().strip()
        uid_gid_host = uid + ":" + gid
        if uid_gid_host != expect_id:
            test.fail("Check file owner/group failed, "
                      "real value is %s, "
                      "expected value is %s" %
                      (uid_gid_host, expect_id))
    ...
    if params.get("privileged", "") == "no":
        ...
        aexpect.ShellSession("runuser -l " + user_name +
                             " -c '" + cmd_run_virtiofsd + "'",
                             auto_close=False,
    ...
    volume_letter = virtio_fs_utils.get_virtiofs_driver_letter(test,
                                                               fs_target,
                                                               session)
    ...
    if params.get("privileged", "") == "yes":
        ...
        dict_ids = {"null": uid_gid_new_user,
    ...
    for test_value, expect_value in dict_ids.items():
        error_context.context("Set host UID:GID=%s to viofs"
                              " service." % test_value, test.log.info)
