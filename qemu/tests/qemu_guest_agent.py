    ...
    def setup(self, test, params, env):
        ...
        if self.start_vm == "yes":
            ...
            if self.params["gagent_src_type"] != "url":
                ...
                error_context.context("Download qemu-guest-agent package from website "
                ...
                process.system(f'wget -qP {gagentrpm_tem_path} {gagentrpm_download_url}')
                s, gagentrpm_tem_path = process.getstatusoutput(f'ls {gagentrpm_tem_path}/{rpm_name}')
                if s != 0:
                    self.test.error("qemu-guest-agent rpm is not exist, maybe it "
                                    "is not successfully downloaded, please take a look ")
                else:
                    self.vm.copy_files_to(gagentrpm_tem_path, gagentrpm_guest_dir)
                    s = session.cmd_status(f'rpm -Uvh --nodeps --force {gagentrpm_guest_dir}/{rpm_name}')
                    if s != 0:
                        self.test.error("qemu-guest-agent rpm couldn't be installed successfully, please take a look")
                    else:
                        restart_cmd = self.params["gagent_restart_cmd"]
                        s_rst, o_rst = session.cmd_status_output(restart_cmd)
                        if s_rst != 0:
