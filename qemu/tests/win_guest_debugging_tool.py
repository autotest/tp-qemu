...
class WinDebugToolTestBasicCheck(WinDebugToolTest):
    ...
    def windegtool_check_networkadapter_collection(self, test, params, env):
        ...
        try:
            ...
            try:
                ...
                test.error(
                    "Failed to create new session after network change: "
                    "%s" % str(e)
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    test.error(
                        "Failed to create new session after recovery: "
                        f"{str(e)}"
                    )
            except Exception as e:
    ...
    def windegtool_check_IO_limits(self, test, params, env):
        ...
        error_context.context(
            "Run script first time to collect baseline IO metrics",
            LOG_JOB.info
        )
        ...
        error_context.context(
            "Run script second time with continuous IO in background",
            LOG_JOB.info
        )
        ...
        io_thread = threading.Thread(
            target=_run_continuous_io,
            args=(session1, stop_event)
        )
        io_thread.start()
