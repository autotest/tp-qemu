...
from .storage_admin import sp_admin
...
class VirtStorageTest(unittest.TestCase):
    ...
    def test_04_1_volumes_define_by_params(self):
        ...
        for name in self.params.objects("images"):
            ...
            unittest.TestCase.assertIsNotNone(
                self, img, "'%s' is None!" % name)

    def test_04_2_img4_backing_file(self):
        img = sp_admin.get_volume_by_name("img4")
        backing_store = img.backing
        unittest.TestCase.assertEqual(
            self,
            backing_store.name,
            "img5",
            "backing store of img4 mismatch!")

    def test_05_1_acqurie_volume(self):
    ...
    def test_05_2_img_backing_file_state(self):
        img = sp_admin.get_volume_by_name("img4")
        backing_store = img.backing
        unittest.TestCase.assertTrue(
            self,
            backing_store.is_allocated,
            "backing store state of img4 mismatch!")

    def test_06_1_compare_volume_info_img1(self):
        img1 = sp_admin.get_volume_by_name("img1")
        info1 = {'capacity': 104857600, 'name': 'img1', 'format': 'qcow2', 'url': 'file:///tmp/avocado/img1',
                 'auth': 'None', 'preallocation': 'off', 'key': '/tmp/avocado/img1', 'path': '/tmp/avocado/img1',
                 'pool': 'DirectoryPool:sp1'}
        unittest.TestCase.assertEqual(
            self, info1, img1.info(), "Volume info of 'img1' mismatch!")

    def test_06_2_img4_encrypt_format(self):
        img = sp_admin.get_volume_by_name("img4")
        #encrypt = img.format.get_param("encrypt")
        encrypt = img.encrypt
        unittest.TestCase.assertEqual(
            self,
            encrypt.format,
            "luks",
            "encrypt format of 'img4' mismatch!")
