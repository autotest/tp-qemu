...
class BlockDevCommitCancel(BlockDevCommitTest):
    ...
    def check_backing_rw(self):
        ...
        if base_image.image_filename.startswith("nbd://"):
            cmd = "lsof -i:%s" % base_image.params["nbd_port"]
        elif base_image.image_filename.startswith("rbd:"):
            # fixme when get the correct ceph communication port
            cmd = "lsof -i:6800"
        ...
        output = process.run(cmd, verbose=True).stdout_text.split("\n")[1].split()[1]
        if output[-1] != "0":
            self.test.fail("backing image is not in ro status")
