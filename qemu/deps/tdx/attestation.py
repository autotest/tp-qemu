...
class TDQuoteHeader:
    ...
    def from_bytes(data):
        ...
        return TDQuoteHeader(version, attkeytype, teetype, qevendorid, userdata)


# A.3.4. TD Attributes
class TDAttr:
    def __init__(
        self, debug_mode, sept_ve_disable, use_svisor_prot_keys, use_key_locker, perfmon
    ):
    ...
    def from_bytes(data):
        ...
        return TDAttr(
            debug_mode, sept_ve_disable, use_svisor_prot_keys, use_key_locker, perfmon
        )
...
class TDQuoteBody:
    ...
    def from_bytes(data):
        ...
        rtmrs = [data[328:376], data[376:424], data[424:472], data[472:520]]
...
class QEReportCertificationData:
    ...
    def from_bytes(data):
        ...
        return QEReportCertificationData(
            qereport, qereporthash, qereportsig, qeauthdata, qecertdata
        )
...
class PCKCertChain:
    ...
    def __repr__(self):
        def cert_name(x509name):
            return "/".join(
                [d[0].decode() + "=" + d[1].decode() for d in x509name.get_components()]
            )
    ...
    def from_bytes(data):
        ...
        while True:
            idx, payload = pem.readPemBlocksFromFile(
                datafile, ("-----BEGIN CERTIFICATE-----", "-----END CERTIFICATE-----")
            )
...
class TDQuoteV4:
    ...
    def verify_tdreport(self):
        vk = ecdsa.VerifyingKey.from_string(self.sig.ecdsaattkey, curve=ecdsa.NIST256p)
        try:
...
def make_quote():
    report = tempfile.mkdtemp(dir="/sys/kernel/config/tsm/report", prefix="report")
