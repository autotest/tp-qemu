...
def run(test, params, env):
    ...
    for o in offset:
        process.run(qemu_img_bench_cmd % (image_stg_name, o),
    ...
    extents_number = re.search(extents_number_pattern,
                               cmd_result.stdout.decode())
    if not extents_number:
        test.fail("Failed to get extents number. "
                  "The output is '%s'." % cmd_result.stdout.decode())
    if int(extents_number.group(1)) >= fragmentation_maximum:
        test.fail("The extents should less than %s, the actual result is %s."
