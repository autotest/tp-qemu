...
def run(test, params, env):
    ...
    def test_migration():
        def check_fio_running():
            ...
            if not procs:
                ...
                # On linux, the count should be numjobs+1
                # because ps lists fio and all its subprocess
                count_fio_processes = params.get_numeric("numjobs") + 1
                if os_type == "windows":
                    # On Windows, the count should be 1
                    # because tasklist only lists the fio.exe process
                    count_fio_processes = 1

                if len(procs) != count_fio_processes:
                    test.fail("Failed to get all running fio process")
    ...
    try:
        ...
        guest_fio_object = start_io(session)
