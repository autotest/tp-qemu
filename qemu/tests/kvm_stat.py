...
def run(test, params, env):
    ...
    def separate_file(file_path, rotate_size_limit):
        rotate_size_limit = int(normalize_data_size(rotate_size_limit, order_magnitude="B", factor=1024))
        num = rotate_time + 1
        while num > 0:
            if utils_misc.wait_for(lambda: os.path.getsize(file_path) > rotate_size_limit, 60, 5, 5):
                process.system('logrotate -v /etc/logrotate.d/kvm_stat')
            num -= 1

    def check_log():
        test.log.info("check if log file num match with rotate")
        check_log_num = int(process.system_output(params.get("check_log_num"), shell=True))
    ...
    def restore_env():
        ...
        tweak_file(r"-s\s([\.\d]+)\s", "-s %s " % initial_kvm_stat_interval, kvm_stat_service_path)
        tweak_file(r"size \s(.*)\s", "size %s" % initial_rotate_size, logrotate_config_file_path)
    ...
    with open(kvm_stat_service_path, 'r') as fd:
        content = fd.read()
    initial_kvm_stat_interval = re.search(r"ExecStart=.*-s\s([\.\d]+)\s", content).group(1)
    ...
    try:
        test.log.info("Adjust the parameter '-s' is %s in %s", kvm_stat_interval, kvm_stat_service_path)
        tweak_file(r"-s %s" % initial_kvm_stat_interval, "-s %s" % kvm_stat_interval, kvm_stat_service_path)
        ...
        tweak_file(r"size %s" % initial_rotate_size, "size %s" % rotate_size_limit, logrotate_config_file_path)
