...
class BlockdevIncbkAddBitmapMaxLenName(BlockdevLiveBackupBaseTest):
    ...
    def __init__(self, test, params, env):
        super(BlockdevIncbkAddBitmapMaxLenName, self).__init__(test,
                                                               params,
                                                               env)
        self._max_len_name = self._make_bitmap_name()

    def _make_bitmap_name(self):
        length = self.params.get_numeric(
            'bitmap_name_max_len') - len(self.params['prefix_name'])
        return self.params['prefix_name'] + process.run(
    ...
    def add_persistent_bitmap(self):
        kargs = {'bitmap_name': self._max_len_name,
