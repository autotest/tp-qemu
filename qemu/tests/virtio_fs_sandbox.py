...
def run(test, params, env):
    ...
    error_context.context("Switch to the common user and create the"
                          " shared dir in home dir.", test.log.info)
    # set fs shared dir
    ...
    process.system("su - %s -c 'mkdir -p %s'" %
                   (common_user, shared_dir), shell=True)

    # set fs socket
    sock_path = os.path.join("/home/" + common_user,
    ...
    error_context.context("Running daemon command %s with user." %
                          cmd_run_virtiofsd,
                          test.log.info)

    virtiofsd_cmd = "runuser -l %s -c '%s'" % (common_user, cmd_run_virtiofsd)
    session = aexpect.ShellSession(virtiofsd_cmd, auto_close=False,
    ...
    try:
        ...
        if not is_windows:
            ...
            if not s:
                test.fail("Should not change file's owner/group because"
