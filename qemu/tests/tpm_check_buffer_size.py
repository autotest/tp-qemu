...
def run(test, params, env):
    ...
    if process.system("rpm -q libtpms-devel", shell=True,
                      ignore_status=True) != 0:
        try:
            process.system(
                "cd /home/ && brew download-build --rpm %s" %
                libtpms_devel_rpm, shell=True)
    ...
    try:
        error_context.base_context(
            "Build and execute test cases", test.log.info)
        for test_case in params.get("test_case").split(";"):
            test_case_o = test_case.split(".")[0]
            build_execute_cmd = params["build_execute_cmd"] % (
                test_case, test_case_o, test_case_o)
            process.system("cd %s && %s" %
                           (repo_dir, build_execute_cmd), shell=True)
    finally:
        process.system("rm -f /home/%s*" % libtpms_devel_rpm,
